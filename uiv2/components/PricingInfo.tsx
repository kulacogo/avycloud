...
const PricingInfo: React.FC<PricingInfoProps> = ({ pricing, isEditing = false, onChange }) => {
  ...
  return (
    <>
      <div className="flex items-baseline space-x-4">
        ...
        {isEditing ? (
          <div className="flex items-center gap-2">
            <input type="number" step="0.01" defaultValue={lowest_price.amount} onBlur={e => setAmount(e.target.value)} className="w-28 bg-[var(--surface)] border border-[var(--border-hover)] rounded px-2 py-1 text-[color:var(--text-primary)]" />
            <input type="text" defaultValue={lowest_price.currency} onBlur={e => setCurrency(e.target.value.toUpperCase())} className="w-20 bg-[var(--surface)] border border-[var(--border-hover)] rounded px-2 py-1 text-[color:var(--text-primary)] uppercase" />
          </div>
        ) : (
          <span id="price-value" className="text-3xl font-bold text-[color:var(--avy-purple-light)]">
      ...
      <div className="mt-1">
        <span className="text-sm text-[color:var(--text-tertiary)]">Confidence: </span>
        {isEditing ? (
          <input type="number" step="0.01" min="0" max="1" defaultValue={price_confidence} onBlur={e => setConfidence(e.target.value)} className="w-24 bg-[var(--surface)] border border-[var(--border-hover)] rounded px-2 py-1 text-[color:var(--text-primary)]" />
        ) : (
          <span className="text-sm font-medium text-[color:white]">{ (price_confidence * 100).toFixed(0) }%</span>
        )}
      </div>

      {(nonLinkSources.length > 0 || linkSources.length > 0) && (
        <div className="mt-4">
          <h4 className="font-semibold text-[color:var(--text-secondary)] mb-2">Evidence sources:</h4>
          <ul id="price-sources" className="space-y-2">
            {nonLinkSources.map((source, index) => (
              <li key={`nonlink-${index}`} className="flex items-center justify-between p-2 bg-[var(--surface)]/50 rounded-md">
                <div className="flex items-center">
                  <LinkIcon className="w-4 h-4 text-[color:var(--text-tertiary)] mr-2" />
            ...
            {linkSources.map((source, index) => (
              <li key={index} className="flex items-center justify-between p-2 bg-[var(--surface)]/50 rounded-md">
                <div className="flex items-center">
                  <LinkIcon className="w-4 h-4 text-[color:var(--text-tertiary)] mr-2" />
                  <a href={source.url} target="_blank" rel="noopener noreferrer" className="text-[color:var(--avy-purple-light)] hover:underline">
                    {source.name}
                  </a>
                </div>
                <span className="font-mono text-[color:var(--text-secondary)]">
      ...
      {lowest_price.last_checked_iso && (
        <small id="price-checked" className="block text-right text-xs text-[color:var(--text-tertiary)] mt-4">
