  ...
  return (
    <div className="rounded-2xl bg-[var(--surface-secondary)]/60 p-4 ring-1 ring-[var(--border)]/60">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <div className="text-sm font-semibold text-[color:var(--text-primary)]">Data coverage</div>
        ...
        <button
          type="button"
          onClick={() => load()}
          className="rounded-xl bg-[var(--surface-hover)]/80 px-3 py-2 text-xs font-semibold text-[color:var(--text-primary)] hover:bg-[var(--surface)] disabled:opacity-60"
          disabled={loading}
        >
      ...
      {error ? (
        <div className="mt-3 rounded-xl bg-[var(--error-bg)] p-3 text-sm text-[color:var(--error)] ring-1 ring-[var(--error-border)]">{error}</div>
      ) : null}

      <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-3">
        <div className="rounded-xl bg-[var(--surface-hover)]/60 p-3 ring-1 ring-[var(--border)]/50">
          <div className="text-xs text-[color:var(--text-tertiary)]">Products</div>
          <div className="mt-1 text-2xl font-bold text-[color:var(--text-primary)]">{total}</div>
          <div className="mt-1 text-xs text-[color:var(--text-tertiary)]">
            Title policy ok:{' '}
            <button
              type="button"
              onClick={() => open('Title policy ok', data?.buckets?.titleOkIds)}
              className="text-[color:var(--text-primary)] underline decoration-[var(--border)] underline-offset-2 hover:decoration-[var(--border-hover)]"
            >
              {titleOk}
            </button>
        ...
        <div className="rounded-xl bg-[var(--surface-hover)]/60 p-3 ring-1 ring-[var(--border)]/50">
          <div className="text-xs text-[color:var(--text-tertiary)]">K‑Typ</div>
          <div className="mt-1 text-2xl font-bold text-[color:var(--text-primary)]">
            <button
              type="button"
              onClick={() => open('K‑Typ set', data?.buckets?.ktypWithValueIds)}
              className="underline decoration-[var(--border)] underline-offset-4 hover:decoration-[var(--border-hover)]"
            >
              {ktyp}
            </button>{' '}
            <span className="text-sm font-semibold text-[color:var(--text-tertiary)]">
        ...
        <div className="rounded-xl bg-[var(--surface-hover)]/60 p-3 ring-1 ring-[var(--border)]/50">
          <div className="text-xs text-[color:var(--text-tertiary)]">GPSR</div>
          <div className="mt-1 text-2xl font-bold text-[color:var(--text-primary)]">
            <button
              type="button"
            ...
            <span className="text-sm font-semibold text-[color:var(--text-tertiary)]">({formatPct(gpsrFullNoPH, total)})</span>
          </div>
          <div className="mt-1 text-xs text-[color:var(--text-tertiary)]">
      ...
      <div className="mt-3 flex flex-wrap items-end gap-3">
        <div className="rounded-xl bg-[var(--surface-hover)]/40 p-3 ring-1 ring-[var(--border)]/40">
          <div className="text-xs font-semibold text-[color:var(--text-primary)]">Price sanity (configurable)</div>
          <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-[color:var(--text-secondary)]">
            <label className="inline-flex items-center gap-2">
              <span className="text-[color:var(--text-tertiary)]">Min</span>
              <input
                value={minPrice}
                onChange={(e) => {
                ...
                placeholder="(off)"
                className="w-20 rounded-lg bg-[var(--surface-secondary)]/40 px-2 py-1 text-[color:var(--text-primary)] ring-1 ring-[var(--border)]/50"
              />
            </label>
            <label className="inline-flex items-center gap-2">
              <span className="text-[color:var(--text-tertiary)]">Max</span>
              <input
                value={maxPrice}
                onChange={(e) => {
                  const v = e.target.value;
            ...
            <button
              type="button"
              onClick={() => load()}
              className="rounded-lg bg-[var(--surface)] px-2.5 py-1 font-semibold text-[color:var(--text-primary)] hover:bg-[var(--surface-secondary)]"
            >
              Apply
            </button>
          </div>
          <div className="mt-2 grid grid-cols-3 gap-2">
            <button
              ...
              className="rounded-lg bg-[var(--surface-secondary)]/40 p-2 text-left ring-1 ring-[var(--border)]/40 hover:bg-[var(--surface-secondary)]/60"
            >
              <div className="text-[11px] text-[color:var(--text-tertiary)]">Missing</div>
              <div className="text-sm font-bold text-[color:var(--text-primary)]">{data?.price?.missingCount ?? 0}</div>
            </button>
            <button
              type="button"
              ...
              className="rounded-lg bg-[var(--surface-secondary)]/40 p-2 text-left ring-1 ring-[var(--border)]/40 hover:bg-[var(--surface-secondary)]/60"
            >
              <div className="text-[11px] text-[color:var(--text-tertiary)]">OK</div>
              <div className="text-sm font-bold text-[color:var(--text-primary)]">{data?.price?.okCount ?? 0}</div>
            </button>
            <button
              type="button"
              ...
              className="rounded-lg bg-[var(--surface-secondary)]/40 p-2 text-left ring-1 ring-[var(--border)]/40 hover:bg-[var(--surface-secondary)]/60"
            >
              <div className="text-[11px] text-[color:var(--text-tertiary)]">Out of range</div>
              <div className="text-sm font-bold text-[color:var(--text-primary)]">{data?.price?.outOfRangeCount ?? 0}</div>
            </button>
          </div>
        </div>

        <div className="flex-1 rounded-xl bg-[var(--surface-hover)]/40 p-3 ring-1 ring-[var(--border)]/40">
          <div className="text-xs font-semibold text-[color:var(--text-primary)]">Main categories</div>
            ...
            {Object.entries(data?.categories?.mainCategoryCounts || {})
              ...
              .map(([cat, count]) => (
                <button
                  ...
                  className="rounded-full bg-[var(--surface-secondary)]/40 px-3 py-1 text-xs font-semibold text-[color:var(--text-primary)] ring-1 ring-[var(--border)]/40 hover:bg-[var(--surface-secondary)]/60"
                >
                  {cat} <span className="text-[color:var(--text-tertiary)]">({count})</span>
                </button>
              ))}
          </div>
          <div className="mt-2 text-[11px] text-[color:var(--text-tertiary)]">Shows top 12; click a chip to open filtered list.</div>
        </div>
      </div>

      <div className="mt-4 rounded-xl bg-[var(--surface-hover)]/40 p-3 ring-1 ring-[var(--border)]/40">
        <div className="text-xs font-semibold text-[color:var(--text-primary)]">GPSR required fields filled (per product, no placeholders)</div>
          ...
          {Array.from({ length: 9 }).map((_, i) => {
            ...
            return (
              <button
                ...
                className="rounded-lg bg-[var(--surface-secondary)]/40 p-2 text-left ring-1 ring-[var(--border)]/40 hover:bg-[var(--surface-secondary)]/60"
              >
                <div className="text-[11px] text-[color:var(--text-tertiary)]">{i}/8</div>
                <div className="text-sm font-bold text-[color:var(--text-primary)]">{count}</div>
                <div className="text-[11px] text-[color:var(--text-tertiary)]">{formatPct(count, histTotal || total)}</div>
              </button>
            );
          })}
        </div>
        <div className="mt-2 text-[11px] text-[color:var(--text-tertiary)]">
          ...
          <span className="text-[color:var(--text-secondary)]">{Number(histIncl['8'] || 0)}</span> / {histInclTotal || total}.
        </div>
      </div>
    </div>
  );
