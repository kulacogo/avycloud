    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <label className="text-xs text-[color:var(--text-secondary)] flex items-center gap-2 mr-1">
          ...
          <button
            ...
            onClick={load}
            className="rounded-xl bg-[var(--surface-hover)]/70 px-3 py-2 text-sm font-semibold text-[color:var(--text-primary)] hover:bg-[var(--surface)] disabled:opacity-50"
          >
            Refresh
          </button>
          <button
            ...
            disabled={loading || saving}
            onClick={save}
            className="rounded-xl bg-[var(--avy-purple)] px-3 py-2 text-sm font-semibold text-[color:white] hover:bg-[var(--avy-purple-hover)] disabled:opacity-50"
          >
            Save
          </button>
        </div>
      ...
      {error && <div className="rounded-xl border border-[var(--error-border)] bg-[var(--error-bg)] p-3 text-sm text-[color:var(--error)]">{error}</div>}
      {loading && <div className="text-sm text-[color:var(--text-secondary)]">Loading…</div>}

      <section className="rounded-2xl border border-[var(--border)] bg-[var(--surface-secondary)]/40 p-4 space-y-3">
        <h3 className="text-sm font-semibold text-[color:var(--text-primary)]">Titel Regeln (Längen + Marketing Wörter)</h3>
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          {(['minLen', 'softMaxLen', 'maxLen', 'mobileMaxLen'] as const).map((k) => (
            <label key={k} className="text-xs text-[color:var(--text-secondary)] space-y-1">
              <span>{k}</span>
              <input
                className="w-full rounded-xl bg-[var(--bg)]/40 border border-[var(--border)] px-3 py-2 text-[color:var(--text-primary)]"
                value={(cfg.title as any)?.[k] ?? ''}
                onChange={(e) => setCfg((prev) => ({ ...prev, title: { ...(prev.title || {}), [k]: Number(e.target.value) } }))}
              />
            </label>
          ))}
          <label className="text-xs text-[color:var(--text-secondary)] space-y-1 md:col-span-2">
      ...
      <section className="rounded-2xl border border-[var(--border)] bg-[var(--surface-secondary)]/40 p-4 space-y-3">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-semibold text-[color:var(--text-primary)]">Titel Kategorien (pro Titel-Kategorie)</h3>
          <button
            type="button"
            className="rounded-xl bg-[var(--surface-hover)]/70 px-3 py-2 text-xs font-semibold text-[color:var(--text-primary)] hover:bg-[var(--surface)]"
            onClick={() =>
              setTitleRows((prev) => [
                ...prev,
        ...
        <div className="overflow-auto rounded-xl ring-1 ring-white/10">
          <table className="min-w-full text-xs">
            <thead className="bg-[var(--bg)]/40 text-[color:var(--text-secondary)]">
              <tr>
            ...
            <tbody className="divide-y divide-white/10">
              {titleRows.map((r, idx) => (
                <tr key={`${r.schemaId}-${idx}`} className="bg-[var(--surface-secondary)]/20">
                  <td className="px-3 py-2">
                    <input
                      className="w-44 rounded-xl bg-[var(--bg)]/40 border border-[var(--border)] px-3 py-2 text-[color:var(--text-primary)]"
                      value={r.schemaId}
                  ...
                  {(['minLen', 'softMaxLen', 'maxLen', 'mobileMaxLen'] as const).map((k) => (
                    <td key={k} className="px-3 py-2">
                      <input
                        className="w-24 rounded-xl bg-[var(--bg)]/40 border border-[var(--border)] px-3 py-2 text-[color:var(--text-primary)]"
                        value={(r as any)[k]}
                  ...
                  <td className="px-3 py-2">
                    <button
                      type="button"
                      className="rounded-xl bg-[var(--error)]/80 px-3 py-1.5 text-xs font-semibold text-[color:white] hover:bg-[var(--error)]"
      ...
      <section className="rounded-2xl border border-[var(--border)] bg-[var(--surface-secondary)]/40 p-4 space-y-3">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-semibold text-[color:var(--text-primary)]">Highlights Regeln (pro Schema)</h3>
          <button
            type="button"
            className="rounded-xl bg-[var(--surface-hover)]/70 px-3 py-2 text-xs font-semibold text-[color:var(--text-primary)] hover:bg-[var(--surface)]"
        ...
        <label className="text-xs text-[color:var(--text-secondary)] flex items-center gap-2">
          <input
            type="checkbox"
          ...
          <table className="min-w-[760px] w-full text-sm">
            ...
            <tbody className="text-[color:var(--text-primary)]">
              {highlightRows.map((row, idx) => (
                <tr key={`${row.schemaId}-${idx}`} className="border-t border-[var(--border)]">
                  {(['schemaId', 'min', 'max', 'minLen', 'maxLen'] as const).map((k) => (
                    <td key={k} className="px-3 py-2">
                      <input
                        className="w-full rounded-lg bg-[var(--bg)]/40 border border-[var(--border)] px-2 py-1 text-[color:var(--text-primary)]"
                        value={(row as any)[k]}
                  ...
                  <td className="px-3 py-2">
                    <button
                      type="button"
                      className="rounded-lg bg-[var(--error)]/20 border border-[var(--error-border)] px-2 py-1 text-xs text-[color:var(--error)] hover:bg-[var(--error)]/30"
              ...
              {!highlightRows.length && (
                <tr>
                  <td className="px-3 py-3 text-[color:var(--text-tertiary)]" colSpan={6}>
      ...
      <section className="rounded-2xl border border-[var(--border)] bg-[var(--surface-secondary)]/40 p-4 space-y-3">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-semibold text-[color:var(--text-primary)]">Attribute Canonical Map (Synonyme → Canonical Key)</h3>
          <button
            type="button"
            className="rounded-xl bg-[var(--surface-hover)]/70 px-3 py-2 text-xs font-semibold text-[color:var(--text-primary)] hover:bg-[var(--surface)]"
        ...
        <div className="overflow-auto rounded-xl border border-[var(--border)]">
          <table className="min-w-[760px] w-full text-sm">
            ...
            <tbody className="text-[color:var(--text-primary)]">
              ...
              {!canonRows.length && (
                <tr>
                  <td className="px-3 py-3 text-[color:var(--text-tertiary)]" colSpan={3}>
      ...
      <section className="rounded-2xl border border-[var(--border)] bg-[var(--surface-secondary)]/40 p-4 space-y-3">
        <h3 className="text-sm font-semibold text-[color:var(--text-primary)]">Version Note</h3>
        <textarea
          rows={2}
          className="w-full rounded-xl bg-[var(--bg)]/40 border border-[var(--border)] px-3 py-2 text-[color:var(--text-primary)] outline-none focus:border-[var(--avy-purple)]"
      ...
      <section className="rounded-2xl border border-[var(--border)] bg-[var(--surface-secondary)]/40 p-4 space-y-3">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-sm font-semibold text-[color:var(--text-primary)]">Pflicht: Initial Run + Delta Sync</h3>
            <p className="text-xs text-[color:var(--text-tertiary)]">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={saving}
              onClick={runApply}
              className="rounded-xl bg-[var(--success)] px-3 py-2 text-sm font-semibold text-[color:white] hover:bg-[var(--success)] disabled:opacity-50"
            >
              Run now
            </button>
            <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <label className="flex flex-col gap-1 text-sm text-[color:var(--text-primary)]">
            <span className="text-[11px] uppercase tracking-wide text-[color:var(--text-tertiary)]">minQty (available)</span>
            <input
              type="number"
              min={1}
              max={9999}
        ...
        <div className="text-xs text-[color:var(--text-secondary)]">
          JobId:{' '}
          <input
            className="ml-2 rounded-lg bg-[var(--bg)]/40 border border-[var(--border)] px-2 py-1 text-[color:var(--text-primary)]"
            value={applyJobId}
        ...
        {applyJob && (
          <pre className="rounded-xl bg-[var(--bg)]/40 border border-[var(--border)] p-3 text-xs text-[color:var(--text-primary)] overflow-auto">
