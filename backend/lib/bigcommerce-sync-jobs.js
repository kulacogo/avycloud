...
const JOBS_COLLECTION = 'bigcommerceSyncJobs';
...
function sanitizeValue(value) {
  ...
  if (isSpecialFirestoreValue(value)) return value;
  if (Array.isArray(value)) {
    return value.map((v) => sanitizeValue(v)).filter((v) => v !== undefined);
  }
...
async function createJob(payload, jobId = null) {
  const base = {
    ...
    stage: 'queued',
    progress: { total: 0, processed: 0, synced: 0, failed: 0 },
    result: null,
    error: null,
    ...payload,
  };
