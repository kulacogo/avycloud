...
- `lib/image-hosts.js` — `classifyImageHost()` / `isCopyrightBlockedHost()`. 40 % der Bildadressen liegen auf Fremdservern; 719 Fotos bei 174 Produkten stammen von Amazon und dürfen NICHT rehostet werden (Urheberrecht), sie müssen ersetzt werden. Audit: `backend/scripts/audit-product-images.js` (read-only).
- `lib/ebay-aspect-name-normalizer.js` + `backend/data/ebay-aspect-synonyms.json` — 48,2 % der gesendeten Merkmalsnamen kennt eBay in der Zielkategorie nicht und erzeugt deshalb keinen Suchfilter (z. B. „Koffermaterial" statt „Material"). NICHT in `filterPatchItemSpecificsForListing` einhängen, solange `backend/data/aspects-full.json` vom 17.02.2026 stammt — auf veraltete Namen umzubenennen ist schlimmer als gar nicht. Erst Katalog-Neusync (`scripts/sync-ebay-required-aspects-full.js`), dann `shadow`, dann `on`. Audit: `scripts/audit-aspect-name-coverage.js`.
- `backend/scripts/probe-ebay-listing-metrics.js` (read-only) — Beobachter/Aufrufe/Verkaufsmenge fehlen in allen 4.828 Spiegel-Dokumenten. Der geplante Weg (`GetMyeBaySelling` mit `DetailLevel=ReturnAll`) steht auf einer ungeprüften API-Annahme; erst messen, dann bauen.
