...
export const fetchProducts = async (options?: { timeoutMs?: number }): Promise<Product[]> => {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      // Project table fields in Firestore, before transferring/decoding the
      // catalog. All products and filter inputs remain; ProductSheet loads
      // the full document separately. Read contract: product-read-models.js.
      response = await fetchWithTimeout(
        `${BACKEND_URL}/api/products?view=table`,
        undefined,
        timeout
      );
      lastError = null;
