...
// Sync product(s) to BigCommerce – creates new products or updates existing ones
export const syncToBigCommerce = async (
  productOrProducts: Product | Product[]
): Promise<{ ok: boolean; results?: Array<{ id: string; status: 'synced' | 'failed'; message?: string; action?: string; bigcommerce_product_id?: any }>; error?: { code: number; message: string } }> => {
  let response: Response | undefined;
  ...
  try {
    ...
    const ids = products.map((p) => p?.id).filter(Boolean) as string[];
    if (!ids.length) {
      return { ok: false, error: { code: 400, message: 'No products selected.' } };
    }

    if (import.meta.env.DEV) {
      console.log('API CALL: /api/bigcommerce/sync/jobs', { count: ids.length });
    }

    response = await fetch(`${BACKEND_URL}/api/bigcommerce/sync/jobs`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    while (Date.now() - startedAt < TIMEOUT_MS) {
      const jobRes = await fetch(`${BACKEND_URL}/api/bigcommerce/sync/jobs/${encodeURIComponent(jobId)}`, {
        method: 'GET',
      });
      const jobPayload = await parseResponse(jobRes);
      if (!jobRes.ok) {
          ...
          error:
            failed.length > 0 || status === 'failed'
              ? {
                code: 502,
                message:
                  failed.map((f: any) => `${f.id}: ${f.message || 'Sync failed'}`).join(' | ') ||
    ...
    console.error('Failed to sync to BigCommerce:', error);
    const errorInfo = extractErrorInfo(error, response);
    return { ok: false, error: errorInfo };
  }
};
