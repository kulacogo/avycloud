...
import AttributeTable from '../product/AttributeTable';
import PricingInfo from '../product/PricingInfo';
...
const ProductSheet: React.FC<ProductSheetProps> = ({ product, onUpdate, onImprove, isImproving }) => {
  ...
  return (
    ...
    >
      {notification && (
        <div className={`fixed top-20 right-8 p-4 rounded-lg shadow-[var(--shadow-lg)] z-50 ${notification.type === 'success' ? 'bg-[var(--success)]' : 'bg-[var(--error)]'} text-[var(--text-primary)]`}>
          {notification.message}
        </div>
      )}

      <div className="space-y-5">
        <header className="p-4 bg-[var(--bg)]/70 border border-[var(--border)] rounded-xl shadow-[var(--shadow-lg)]">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div className="flex-1 min-w-0">
              {isEditing ? (
                ...
                <h1
                  className={`text-2xl sm:text-3xl font-bold break-words leading-tight ${
                    hasQualityIssue('identification.name') ? 'text-[var(--error)]' : ''
                  }`}
                  style={{ wordBreak: 'break-word' }}
                >
              ...
              <p id="p-brand-cat" className="text-[var(--text-tertiary)] mt-1">
                <input
                ...
                {' · '}
                {isEditing ? (
                  <span className="inline-flex flex-col gap-1 text-xs text-[var(--text-secondary)]">
                    <input
                      value={categoryQuery}
                      onChange={(e) => setCategoryQuery(e.target.value)}
                      className={`bg-transparent border-b outline-none ${
                        hasQualityIssue('details.categoryId') ? 'border-[var(--error)]' : 'border-[var(--avy-purple)]'
                      }`}
                    ...
                    <select
                      ...
                      className="bg-[var(--surface)] border border-[var(--border)] rounded-md px-2 py-1 text-xs text-[var(--text-secondary)]"
                    >
                    ...
                    {categoryLoading && <span className="text-[10px] text-[var(--text-tertiary)]">Lade Kategorien…</span>}
                    {categoryError && <span className="text-[10px] text-[var(--error)]">{categoryError}</span>}
                  </span>
                ) : (
                  <span className="text-[var(--avy-purple-light)]">
                    {getProductDisplayCategory(localProduct)}
                    {localProduct.details?.categoryId ? (
                      <span className="text-[var(--text-tertiary)]"> ({localProduct.details.categoryId})</span>
                    ) : null}
                  </span>
                )}
              </p>

              <div className="flex flex-wrap items-center gap-2 text-[11px] text-[var(--text-tertiary)] mt-2">
                <span>
                  SKU:{' '}
                ...
                <button
                  ...
                  className="flex items-center px-3 py-1.5 bg-[var(--surface-hover)] text-[var(--text-primary)] rounded-full hover:bg-[var(--surface-hover)] disabled:opacity-40"
              ...
              {isEditing ? (
                <div className="mt-3">
                  ...
                  <textarea
                    ...
                    className={`w-full bg-[var(--surface)] border rounded-lg p-2 text-xs text-[var(--text-secondary)] ${
                  ...
                  />
                  <p className="text-[11px] text-[var(--text-tertiary)] mt-1">{t('input.barcodes.hint')}</p>
                  <div className="text-[11px] mt-1">
                    {editingBarcodeSummary.hasValid ? (
                      ...
                      <span className="text-[var(--warning)]">{t('sheet.barcodes.statusMissing')}</span>
                    )}
                  </div>
                </div>
              ) : (
                <div className="mt-1">
                  <p id="p-barcodes" className="text-xs text-[var(--text-tertiary)]">
                    {t('common.barcodeLabel')}: {localProduct.identification.barcodes?.join(', ') || t('common.na')}
                  </p>
                  <div className="flex flex-wrap gap-2 text-[11px] text-[var(--text-tertiary)] mt-1">
                    {currentBarcodeSummary.gtin && (
                      <span className="px-2 py-0.5 rounded-full bg-[var(--success)]/10 text-[var(--success)] border border-[var(--success)]/30">
              ...
              <div className="mt-4 border border-[var(--border)] rounded-lg bg-[var(--bg)]/40 p-3">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-semibold text-[var(--text-secondary)]">Quality Gate</span>
                      {qualityGate?.checked_at_iso ? (
                        <span className="text-[11px] text-[var(--text-tertiary)]">
                          {new Date(qualityGate.checked_at_iso).toLocaleString('de-DE')}
                        </span>
                      ) : (
                        <span className="text-[11px] text-[var(--text-tertiary)]">noch nicht geprüft</span>
                      )}
                    ...
                    {qualityGate?.summary && <div className="text-[12px] text-[var(--text-secondary)] mt-1">{qualityGate.summary}</div>}
                  </div>
                  <button
                    type="button"
                    ...
                    className={`px-3 py-1.5 rounded-md text-xs font-semibold ${
                      qualityHasErrors
                        ? 'bg-[var(--error)] text-[var(--text-primary)] hover:bg-[var(--error)]'
                        : qualityHasWarns
                          ? 'bg-[var(--warning)] text-[var(--text-primary)] hover:bg-[var(--warning)]'
                ...
                {qualityMessage && <div className="text-[11px] text-[var(--text-tertiary)] mt-2">{qualityMessage}</div>}
                ...
                {qualityIssues.length > 0 && (
                  <div className="mt-2 space-y-1">
                    ...
                    {qualityIssues.length > 6 && (
                      <div className="text-[11px] text-[var(--text-tertiary)]">… und {qualityIssues.length - 6} weitere</div>
                    )}
                  </div>
                )}
                ...
                {qualityGate?.evidence?.query && (
                  <div className="mt-3 border-t border-[var(--border)] pt-2">
                    <div className="text-[11px] text-[var(--text-tertiary)]">Web-Evidenz Query:</div>
                    <div className="text-[12px] text-[var(--text-secondary)] font-mono break-words">{qualityGate.evidence.query}</div>
                    {Array.isArray(qualityGate?.evidence?.pages) && qualityGate.evidence.pages.length > 0 && (
                      <div className="mt-2 space-y-1">
                        <div className="text-[11px] text-[var(--text-tertiary)]">Quellen:</div>
                        {qualityGate.evidence.pages.slice(0, 3).map((p: any, idx: number) => (
                          <div key={`qg-evidence-${idx}`} className="text-[12px]">
                            <a
                              ...
                              target="_blank"
                              rel="noopener noreferrer"
                              className="text-[var(--avy-purple-light)] hover:underline break-words"
                            >
                              {p?.title || p?.url}
                            </a>
                            {p?.via && <span className="ml-2 text-[11px] text-[var(--text-tertiary)]">({p.via})</span>}
                          </div>
                        ))}
                        {qualityGate.evidence.pages.length > 3 && (
                          <div className="text-[11px] text-[var(--text-tertiary)]">… und {qualityGate.evidence.pages.length - 3} weitere</div>
                        )}
                      </div>
                    )}
                  </div>
            ...
            <div className="actions flex flex-col sm:flex-row flex-wrap gap-2 w-full sm:w-auto justify-end">
              <button
                ...
                className={`flex items-center justify-center px-4 py-2 font-medium rounded-lg transition-colors w-full sm:w-auto ${isEditing ? 'bg-[var(--surface-hover)] text-[var(--text-primary)] hover:bg-[var(--surface-hover)]' : 'bg-[var(--avy-purple)] text-[var(--text-primary)] hover:bg-[var(--avy-purple-hover)]'
                  }`}
              >
              ...
              <button
                ...
                onClick={handleSave}
                disabled={isSaving}
                className="flex items-center justify-center px-4 py-2 bg-[var(--success)] text-[var(--text-primary)] font-medium rounded-lg hover:bg-[var(--success)] transition-colors disabled:bg-[var(--success)]/60 disabled:cursor-not-allowed w-full sm:w-auto"
              >
              ...
              {onImprove && (
                <button
                  ...
                  className="flex items-center justify-center px-4 py-2 bg-[var(--avy-purple)] text-[var(--text-primary)] font-medium rounded-lg hover:bg-[var(--avy-purple-hover)] transition-colors disabled:opacity-60 w-full sm:w-auto"
                >
              ...
              <button
                ...
                className="flex items-center justify-center gap-2 px-4 py-2 bg-[var(--avy-purple)] text-[var(--text-primary)] font-medium rounded-lg hover:bg-[var(--avy-purple-hover)] transition-colors disabled:bg-[var(--avy-purple)]/60 disabled:cursor-wait w-full sm:w-auto"
              >
                {isPublishingEbay ? (
              ...
              <div className="mt-4 space-y-3">
                <div className="flex flex-col sm:flex-row gap-3">
                  <input
                    ...
                    className="flex-1 bg-[var(--surface-hover)] border border-[var(--border)] rounded-lg p-2 text-[var(--text-secondary)]"
                    value={newImageUrl}
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-[var(--surface-hover)] rounded-lg text-[var(--text-primary)] font-semibold hover:bg-[var(--surface-hover)] transition-colors disabled:opacity-50"
                ...
                <div
                  className={`rounded-xl border-2 border-dashed p-4 text-center text-xs sm:text-sm transition-colors ${isUploadDragActive ? 'border-[var(--avy-purple)] bg-[var(--surface)]/60' : 'border-[var(--border)] bg-[var(--bg)]/40'}`}
                  onDragOver={handleUploadDragOver}
                  onDragEnter={handleUploadDragOver}
                  onDragLeave={handleUploadDragLeave}
                  onDrop={handleUploadDrop}
                >
                  <p className="text-sm font-semibold text-[var(--text-primary)]">{t('sheet.upload.dragTitle')}</p>
                  <p className="text-[var(--text-tertiary)] mt-1">{t('sheet.upload.dragHint')}</p>
                  <div className="mt-3 flex items-center justify-center gap-2 text-[var(--text-tertiary)] text-xs uppercase tracking-wide">
                    <span>{t('sheet.upload.or')}</span>
                    <label className="cursor-pointer rounded-full border border-[var(--border)] px-3 py-1 text-[var(--text-primary)]">
                  ...
                  <p className="text-[11px] text-[var(--text-tertiary)] mt-2">{t('sheet.upload.support')}</p>
                </div>
              </div>
            )}
            {isEditing && (
              <div className="mt-4 pt-4 border-t border-[var(--border)] space-y-3">
                <div>
                  ...
                  {referenceImages.length ? (
                    ...
                    <p className="text-xs text-[var(--warning)]">{t('sheet.ai.noReference')}</p>
                  )}
                </div>
                <button
                  onClick={handleGenerateImages}
                  disabled={isGeneratingImages || !selectedReferenceImage}
                  className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-[var(--avy-purple)] text-[var(--text-primary)] font-semibold rounded-xl hover:bg-[var(--avy-purple-hover)] transition-all disabled:opacity-40 shadow-[var(--shadow-lg)]"
                >
                  {isGeneratingImages ? <Spinner className="w-5 h-5 text-[var(--text-primary)]" /> : <MagicIcon className="w-5 h-5" />}
                  <span>{isGeneratingImages ? t('sheet.ai.running') : t('sheet.ai.cta')}</span>
                </button>
                <p className="text-xs text-[var(--text-tertiary)] text-center">{t('sheet.ai.helper')}</p>
              </div>
            )}
          </div>
          <section id="highlights" className="md:col-span-3 p-4 bg-[var(--bg)]/70 border border-[var(--border)] rounded-xl shadow-[var(--shadow-lg)]">
            <h3 className="text-lg font-semibold mb-2 text-[var(--text-primary)]">{t('sheet.highlights')}</h3>
            {isEditing ? (
              <textarea
                ...
                className="w-full min-h-[110px] bg-[var(--surface)] border border-[var(--border)] rounded-lg p-3 text-[var(--text-secondary)]"
              />
            ) : highlightList.length ? (
              <ul className="space-y-2 list-disc list-inside text-[var(--text-secondary)] text-sm">
                {highlightList.map((feature, index) => (
                  <li key={index}>{feature}</li>
                ))}
              </ul>
            ) : (
              <p className="text-sm text-[var(--text-tertiary)]">{t('sheet.highlights.empty')}</p>
            )}
          </section>
        </div>

        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          <section id="description" className="p-4 bg-[var(--surface)] rounded-lg shadow-[var(--shadow-lg)] h-full">
            <h3 className="text-xl font-semibold mb-3 text-[var(--text-primary)]">{t('sheet.description')}</h3>
            {isEditing ? (
              <textarea
                defaultValue={localProduct.details.short_description}
                onBlur={(e) => handleFieldChange('details.short_description', e.target.value)}
                className="w-full min-h-[120px] bg-[var(--surface-hover)] border border-[var(--border)] rounded-lg p-3 text-[var(--text-secondary)]"
              />
            ) : (
              <p className="text-[var(--text-secondary)] leading-relaxed text-sm sm:text-base">{descriptionText}</p>
            )}
          </section>

          <section id="gpsr" className="p-4 bg-[var(--surface)] rounded-lg shadow-[var(--shadow-lg)] h-full">
            <div className="flex items-center justify-between gap-3">
              <h3 className="text-xl font-semibold mb-3 text-[var(--text-primary)]">GPSR</h3>
              {!hasAnyGpsr && !isEditing ? (
                <span className="text-[11px] px-2 py-0.5 rounded-full bg-[var(--surface-hover)]/60 text-[var(--text-secondary)] border border-[var(--border)]/40">
                  leer
                </span>
              ) : null}
            </div>
                  ...
                  <div key={key} className="flex flex-col gap-1">
                    ...
                    {isEditing ? (
                      <input
                        ...
                        className="w-full bg-[var(--surface-hover)] border border-[var(--border)] rounded-lg px-3 py-2 text-[var(--text-secondary)] text-sm"
                        placeholder="—"
                      />
                    ) : (
                      <div className="text-sm text-[var(--text-secondary)] break-words">
                        {value ? value : <span className="text-[var(--text-tertiary)]">—</span>}
                      </div>
                    )}
                  </div>
                );
          ...
          <section id="attributes" className="p-4 bg-[var(--surface)] rounded-lg shadow-[var(--shadow-lg)] h-full">
            <h3 className="text-xl font-semibold mb-4 text-[var(--text-primary)]">{t('sheet.attributes')}</h3>

            <div className="mb-4 rounded-lg border border-[var(--border)] bg-[var(--bg)]/60 p-3">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <div className="flex items-center gap-2">
                    <div className="text-sm font-semibold text-[var(--text-secondary)]">K-Typ</div>
                    {requiresKTyp && !ktypValue && (
                      <span className="text-[11px] px-2 py-0.5 rounded-full bg-[var(--warning)]/10 text-[var(--warning)] border border-[var(--warning)]/30">
              ...
              {isEditing ? (
                <textarea
                  ...
                  className="mt-3 w-full min-h-[70px] bg-[var(--surface)] border border-[var(--border)] rounded-lg p-2 text-[var(--text-secondary)]"
                />
              ) : (
                <div className="mt-3 text-sm text-[var(--text-secondary)] whitespace-pre-wrap break-words">
                  {ktypValue ? ktypValue : <span className="text-[var(--text-tertiary)]">—</span>}
                </div>
              )}
            </div>

            {localProduct?.ops?.data_quality?.ktype_enrich_v1 ? (
              <details className="mb-4 rounded-lg border border-[var(--border)] bg-[var(--bg)]/60 p-3">
                ...
                <pre className="mt-3 overflow-auto whitespace-pre-wrap text-xs text-[var(--text-secondary)]">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          <section id="pricing" className="p-4 bg-[var(--surface)] rounded-lg shadow-[var(--shadow-lg)] h-full">
            <h3 className="text-xl font-semibold mb-4 text-[var(--text-primary)]">{t('sheet.pricing')}</h3>
            <PricingInfo
          ...
          <section id="storage" className="p-4 bg-[var(--surface)] rounded-lg shadow-[var(--shadow-lg)] h-full">
            <h3 className="text-xl font-semibold mb-4 text-[var(--text-primary)]">{t('sheet.storage')}</h3>
            {binsLoading ? (
              <p className="text-[var(--text-tertiary)] text-sm mb-3">{t('sheet.storage.loading')}</p>
            ) : binsError ? (
              <p className="text-[var(--error)] text-sm mb-3">{binsError}</p>
            ) : productBins.length ? (
              <div className="mb-4 space-y-2 max-h-56 overflow-auto pr-1">
                {productBins.map((bin) => (
                  <div key={bin.code} className="rounded border border-[var(--border)] bg-[var(--bg)]/60 px-3 py-2 text-sm text-[var(--text-secondary)]">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div className="font-semibold text-[var(--text-primary)]">{bin.code}</div>
                      <div className="text-xs text-[var(--text-tertiary)]">
                        Zone {bin.zone} · Etage {bin.etage} · Gang {bin.gang} · Regal {bin.regal} · Ebene {bin.ebene}
                      </div>
                      <div className="text-xs text-[var(--text-secondary)]">Menge {bin.quantity ?? bin.productCount ?? 0}</div>
                    </div>
                  </div>
                ))}
              </div>
            ) : (
              <p className="text-[var(--text-tertiary)] text-sm mb-3">{t('sheet.storage.none')}</p>
            )}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                <label className="block text-xs text-[var(--text-tertiary)] mb-1">{t('sheet.storage.binLabel')}</label>
                <input
                  value={binCodeInput}
              ...
              <div>
                <label className="block text-xs text-[var(--text-tertiary)] mb-1">{t('sheet.storage.quantity')}</label>
                <input
                  type="number"
                  min={1}
                  value={binQuantity}
            ...
            <div className="flex flex-wrap gap-3 mt-4">
              <button
                ...
                className="px-4 py-2 bg-[var(--avy-purple)] text-[var(--text-primary)] rounded hover:bg-[var(--avy-purple-hover)] disabled:opacity-40"
              >
              ...
              {binCodeInput && (
                <button
                  ...
                  className="px-4 py-2 bg-[var(--error)] text-[var(--text-primary)] rounded hover:bg-[var(--error)]"
                >
        ...
        {false && (
        <section className="p-4 bg-[var(--surface)] rounded-lg shadow-[var(--shadow-lg)] space-y-3">
          <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            <div>
              <h3 className="text-xl font-semibold text-[var(--text-primary)]">{t('sheet.inventory.title')}</h3>
            ...
            <div className="flex flex-wrap gap-2">
              <button
                ...
                className="inline-flex items-center gap-2 rounded-xl border border-[var(--border)] px-3 py-1.5 text-sm font-semibold text-[var(--text-primary)] hover:bg-[var(--surface-hover)] transition-colors disabled:opacity-50"
              >
              ...
              <button
                ...
                className="inline-flex items-center gap-2 rounded-xl border border-[var(--border)] px-3 py-1.5 text-sm font-semibold text-[var(--text-primary)] hover:bg-[var(--surface-hover)] transition-colors disabled:opacity-60"
              >
          ...
          <div className="space-y-2">
            <label className="block text-xs text-[var(--text-tertiary)] uppercase tracking-wide">
            ...
            <select
              ...
              className="w-full rounded-xl border border-[var(--border)] bg-[var(--bg)] px-3 py-2 text-sm text-[var(--text-primary)] disabled:opacity-50"
            >
          ...
          {inventoryMessage && <p className="text-xs text-[var(--text-tertiary)]">{inventoryMessage}</p>}
        </section>
        )}

        <section className="p-4 bg-[var(--surface)] rounded-lg shadow-[var(--shadow-lg)]">
          <h3 className="text-xl font-semibold mb-4 text-[var(--text-primary)]">{t('sheet.actions.title')}</h3>
          <div className="actions flex flex-wrap gap-4 items-center">
            ...
            <button
              ...
              onClick={handleSync}
              disabled={isSyncing}
              className="flex items-center justify-center px-4 py-2 bg-[var(--surface-hover)] text-[var(--text-secondary)] font-semibold rounded-lg hover:bg-[var(--surface-hover)] transition-colors disabled:bg-[var(--surface)] disabled:cursor-wait"
            >
            ...
            <button
              ...
              onClick={handlePublishToEbay}
              disabled={isPublishingEbay}
              className="flex items-center justify-center px-4 py-2 bg-[var(--avy-purple)] text-[var(--text-primary)] font-semibold rounded-lg hover:bg-[var(--avy-purple-hover)] transition-colors disabled:bg-[var(--avy-purple)]/60 disabled:cursor-wait"
            >
              {isPublishingEbay ? (
