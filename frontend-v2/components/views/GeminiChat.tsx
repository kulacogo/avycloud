...
const AssistantChat: React.FC<AssistantChatProps> = ({ product, onApplyDatasheetChange, onAddImages }) => {
  ...
  return (
    <ChatContainer onFilesDropped={handleFilesAdded}>
      <header className="flex items-center justify-between border-b border-[var(--border)] px-4 py-3 text-xs uppercase tracking-wide text-[var(--text-tertiary)]">
        <div className="flex items-center gap-2 text-[var(--text-primary)]">
          <SparklesIcon className="h-4 w-4 text-[var(--avy-purple)]" />
          <span className="font-semibold text-sm text-[var(--text-primary)]">{t('chat.header.title')}</span>
          <span className="text-[11px] text-[var(--text-tertiary)]">{t('chat.header.subtitle')}</span>
        </div>
        <button
          type="button"
          onClick={() => {
          ...
          className="rounded-full border border-[var(--border)] px-3 py-1 text-[11px] text-[var(--text-secondary)] hover:border-[var(--avy-purple)] hover:text-[var(--text-primary)] transition-colors duration-150"
      ...
      <div className="flex flex-1 min-h-0 flex-col gap-3 px-4 py-3">
        <div ref={chatBodyRef} className="flex-1 min-h-0 space-y-3 overflow-y-auto pr-1">
          ...
          {isLoading && (
            <div className="flex justify-start">
              <div className="flex items-center gap-2 rounded-xl bg-[var(--surface)] border border-[var(--border)] px-4 py-3 text-sm text-[var(--text-secondary)]">
                <Spinner size="sm" />
                {t('chat.ui.thinking')}
              </div>
            </div>
        ...
        {(pendingChanges.length > 0 || pendingImages.length > 0 || serpInsights.length > 0) && (
          <div className="space-y-4 border-t border-[var(--border)] pt-3 text-xs text-[var(--text-secondary)]">
            {pendingChanges.length > 0 && (
              <details className="rounded-xl border border-[var(--border)] bg-[var(--surface-secondary)]">
                <summary className="flex cursor-pointer items-center justify-between px-3 py-2 text-[11px] font-semibold uppercase tracking-wide text-[var(--text-secondary)]">
                  ...
                  {pendingChanges.map((item) => (
                    <div key={item.id} className="rounded-xl border border-[var(--border)] bg-[var(--surface)] p-3">
                      ...
                      <button
                        ...
                        className="mt-2 rounded-full bg-[var(--avy-purple)] px-3 py-1 text-[11px] font-semibold text-white hover:bg-[var(--avy-purple-hover)] disabled:cursor-wait disabled:opacity-60 transition-colors duration-150"
                      >
                ...
                <div className="space-y-2 p-3">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="rounded-full bg-[var(--avy-purple)] px-3 py-1 text-[11px] font-semibold text-white hover:bg-[var(--avy-purple-hover)] transition-colors duration-150"
                    >
                      Alle hinzufügen
                    </button>
                  </div>
                  ...
                  {pendingImages.map((item) => (
                    <div
                      key={item.id}
                      className="flex min-w-[160px] max-w-[160px] flex-col gap-2 rounded-xl border border-[var(--border)] bg-[var(--surface)] p-2"
                    >
                      <img
                      ...
                      />
                      {item.rationale && <p className="text-[11px] text-[var(--text-tertiary)] line-clamp-2">{item.rationale}</p>}
                      <button
                        type="button"
            ...
            {serpInsights.length > 0 && (
              <details className="rounded-xl border border-[var(--border)] bg-[var(--surface-secondary)]">
                ...
                <div className="space-y-2 p-3 text-[11px] text-[var(--text-secondary)]">
                  {serpInsights.map((entry, index) => (
                    <div key={`${entry.engine}-${index}`} className="rounded-xl border border-[var(--border)] bg-[var(--surface)] p-3">
                      <div className="flex items-center justify-between text-[var(--text-primary)]">
                        <span className="font-semibold">{entry.engine}</span>
                        <span className="text-[var(--text-tertiary)]">{entry.query}</span>
                      </div>
                      {entry.error && <p className="mt-1 text-[var(--error)]">{entry.error}</p>}
                      {!entry.error &&
                        entry.summary?.slice(0, 2).map((item, idx) => (
                          <div key={idx} className="mt-1">
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              className="text-[var(--avy-purple)] underline hover:text-[var(--avy-purple-hover)] transition-colors duration-150"
                            >
                              {item.title || item.url}
                            </a>
                            {item.price && <span className="ml-1 text-[var(--text-secondary)]">{String(item.price)}</span>}
                            {item.source && <span className="ml-1 text-[var(--text-tertiary)]">({item.source})</span>}
                          </div>
                        ))}
                    </div>
                  ))}
      ...
      <div className="space-y-3 border-t border-[var(--border)] px-4 py-4">
        <details className="rounded-xl border border-[var(--border)] bg-[var(--surface-secondary)] p-3">
          <summary className="cursor-pointer select-none text-[11px] font-semibold uppercase tracking-wide text-[var(--text-secondary)]">
            Assistant Optionen
          </summary>
          <div className="mt-3 grid grid-cols-1 gap-3">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div className="flex flex-wrap gap-2 text-[12px]">
                {[
                  ...
                  <button
                    ...
                    className="rounded-full border border-[var(--border)] bg-[var(--surface-secondary)] px-3 py-1 text-[var(--text-secondary)] hover:border-[var(--avy-purple)] hover:text-[var(--text-primary)] transition-colors duration-150"
              ...
              <button
                ...
                className="rounded-full bg-[var(--surface)] border border-[var(--border)] px-3 py-1 text-[12px] text-[var(--text-secondary)] hover:border-[var(--border-hover)] transition-colors duration-150"
            ...
            <div className="flex flex-wrap gap-2 text-[12px]">
              {[
                ...
                <label
                  key={key}
                  className="inline-flex items-center gap-2 rounded-full border border-[var(--border)] bg-[var(--surface-secondary)] px-3 py-1 text-[var(--text-secondary)] hover:border-[var(--avy-purple)] cursor-pointer transition-colors duration-150"
                >
                  <input
                    type="checkbox"
            ...
            <div className="flex flex-wrap gap-2 text-[12px]">
              <label className="inline-flex items-center gap-2 rounded-full border border-[var(--border)] bg-[var(--surface-secondary)] px-3 py-1 text-[var(--text-secondary)] hover:border-[var(--avy-purple)] transition-colors duration-150">
                Fetch-Seiten
                <select
                  value={promptConfig.maxPagesToFetch}
                  onChange={(e) => setPromptConfig((prev) => ({ ...prev, maxPagesToFetch: Number(e.target.value) }))}
                  className="rounded-md bg-[var(--surface)] border border-[var(--border)] px-2 py-1 text-[var(--text-secondary)]"
                >
              ...
              <button
                ...
                className="rounded-full bg-[var(--avy-purple)] px-3 py-1 text-[12px] font-semibold text-white hover:bg-[var(--avy-purple-hover)] transition-colors duration-150"
              >
                Jetzt ausführen
              </button>
            </div>
