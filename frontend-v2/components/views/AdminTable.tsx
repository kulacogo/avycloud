...
const SyncStatusBadge: React.FC<{ status: SyncStatus }> = ({ status }) => {
  ...
  const statusMap = {
    synced: 'bg-[var(--success)]/10 text-[var(--success)]',
...
const AdminTable: React.FC<AdminTableProps> = ({
    ...
    const baseRenderers: ColumnDefinition[] = [
      {
        ...
        widthClass: 'w-20',
        render: ({ product }) => (
          <div className="w-12 h-12 rounded-md overflow-hidden bg-[var(--surface-hover)] flex items-center justify-center text-xs text-[var(--text-tertiary)]">
        ...
        render: ({ product, onSelectProduct: handleSelect }) => (
          <div>
            <a
              ...
              className="font-medium text-[var(--avy-purple-light)] hover:underline"
            >
              {product.identification?.name || '—'}
            </a>
            <div className="text-sm text-[var(--text-tertiary)]">{product.identification?.brand || '—'}</div>
          </div>
        ),
      },
      {
        ...
        defaultVisible: true,
        render: ({ product }) => <span className="text-[var(--text-secondary)]">{getProductDisplayCategory(product)}</span>,
      },
      {
        id: 'sku',
        label: t('table.sku'),
        ...
        defaultVisible: true,
        render: ({ product }) => (
          <div className="text-[var(--text-secondary)] text-sm font-mono leading-tight whitespace-nowrap">
      ...
      },
      {
        ...
        defaultVisible: true,
        render: ({ product }) => (
          <div className="text-[var(--text-secondary)] text-sm font-mono leading-tight">{primaryBarcode(product)}</div>
        ),
      },
      {
        id: 'price',
      ...
      },
      {
        ...
        render: ({ product }) => (
          <div className="flex flex-col leading-tight">
            <span className="font-semibold text-[var(--text-primary)] text-center block">{getProductQuantity(product)}</span>
          </div>
        ),
      },
      {
        ...
        render: ({ product }) => {
          ...
          if (pending <= 0) {
            return <span className="text-[var(--text-tertiary)] text-sm">0</span>;
          }
          return (
        ...
        render: ({ product }) =>
          primaryBin(product) ? (
            <div className="flex flex-col text-sm text-[var(--text-secondary)]">
              <span className="font-mono text-base text-[var(--text-primary)]">{primaryBin(product)}</span>
            </div>
          ) : (
            <span className="text-[var(--text-tertiary)]">{t('table.noBin')}</span>
          ),
      },
      {
        id: 'baselinker',
          ...
          return (
            viewItemUrl ? (
              <a
                ...
                className="inline-flex items-center justify-center rounded-full bg-[var(--avy-purple-glow)] px-2 py-0.5 text-xs font-semibold text-[var(--avy-purple-light)] hover:bg-[var(--avy-purple-glow)] hover:text-[var(--text-primary)]"
              >
                gelistet
              </a>
            ) : (
              <span
                ...
                className="inline-flex items-center justify-center rounded-full bg-[var(--surface-hover)] px-2 py-0.5 text-xs font-semibold text-[var(--text-tertiary)]"
              >
                —
              </span>
            )
      ...
      },
      {
        ...
        render: ({ product }) => {
          ...
          const raw =
            ...
            null;
          if (!raw) return <span className="text-[var(--text-tertiary)]">Keine Daten</span>;
          const date = new Date(raw);
          if (Number.isNaN(date.getTime())) return <span className="text-[var(--text-tertiary)]">Unbekannt</span>;
          return <span className="text-[var(--text-secondary)] text-sm">{date.toLocaleString('de-DE')}</span>;
        },
      },
      {
        id: 'syncStatus',
      ...
      },
      {
        ...
        render: ({ product }) => <span className="text-[var(--text-secondary)] text-sm">{product.ops.revision}</span>,
      },
    ];
  ...
  useEffect(() => {
    ...
    row.classList.add('ring-2', 'ring-[var(--avy-purple)]', 'ring-offset-2', 'ring-offset-[var(--surface)]');
    const timeout = window.setTimeout(() => {
      row.classList.remove('ring-2', 'ring-[var(--avy-purple)]', 'ring-offset-2', 'ring-offset-[var(--surface)]');
    }, 2000);
    return () => {
  ...
  const SortableHeader: React.FC<{ sortKey?: string; children: React.ReactNode; widthClass?: string }> = ({
    ...
    if (!sortKey) {
      return (
        <th className={`p-3 text-xs font-semibold uppercase tracking-wide text-[var(--text-secondary)] whitespace-nowrap ${widthClass || ''}`}>
          {children}
        </th>
      );
    }
    return (
      <th
        className={`p-3 cursor-pointer text-xs font-semibold uppercase tracking-wide text-[var(--text-secondary)] whitespace-nowrap ${widthClass || ''}`}
  ...
  const filterControlClass = 'p-2 text-sm bg-[var(--surface)] border border-[var(--border)] rounded-lg text-[var(--text-primary)]';
  const filterButtonClass =
    'w-full p-2 text-sm bg-[var(--surface)] border border-[var(--border)] rounded-lg text-[var(--text-primary)] text-left';
  const menuItemClass =
    'w-full text-left px-3 py-2 text-sm text-[var(--text-primary)] hover:bg-[var(--surface)] rounded-lg transition';
    ...
    <>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2">
        ...
        <div className="relative">
          ...
          {categoryFilterOpen && (
            <div className="absolute z-30 mt-2 w-[360px] max-w-[90vw] rounded-xl border border-[var(--border)] bg-[var(--bg)] p-3 shadow-[var(--shadow-lg)]">
              <div className="flex items-center justify-between mb-2">
                <p className="text-xs font-semibold uppercase tracking-wide text-[var(--text-secondary)]">Kategorien</p>
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    onClick={() => setFilterCategorySelection([])}
                    className="text-xs text-[var(--avy-purple-light)] hover:underline"
                  >
                    Alle
                  </button>
                  <button
              ...
              <div className="max-h-72 overflow-y-auto space-y-2 pr-1">
                {categoryTree.map((node) => {
                  ...
                  return (
                    <div key={node.top} className="rounded-md border border-[var(--border)] bg-[var(--bg)]/40">
                      <label className="flex items-center gap-2 px-2 py-2 text-sm text-[var(--text-primary)]">
                        <input
                          type="checkbox"
                        ...
                        <span className="text-xs text-[var(--text-tertiary)]">({node.count})</span>
                      </label>
                        ...
                        <div className="border-t border-[var(--border)] px-2 py-2 space-y-1">
                          {node.children.map((c) => {
                            ...
                            return (
                              <label
                                key={key}
                                className="flex items-center gap-2 pl-5 pr-2 py-1 text-sm text-[var(--text-secondary)]"
                              >
                                <input
                                  type="checkbox"
                                ...
                                <span className="text-xs text-[var(--text-tertiary)]">({c.count})</span>
                              </label>
                            );
                          })}
                        </div>
      ...
      <details className="rounded-lg border border-[var(--border)] bg-[var(--bg)]/40">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button
            ...
            className="rounded-md border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-xs font-semibold text-[var(--text-primary)] hover:border-[var(--border)]"
          >
        ...
        <details className="relative">
          <summary className="cursor-pointer select-none list-none [&::-webkit-details-marker]:hidden rounded-md border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-xs font-semibold text-[var(--text-primary)] hover:border-[var(--border)]">
            Tools
          </summary>
          <div className="absolute right-0 mt-2 w-[340px] max-w-[90vw] rounded-xl border border-[var(--border)] bg-[var(--bg)] p-1 shadow-[var(--shadow-lg)] z-30">
            ...
            {mode === 'inventory' ? (
              <>
                ...
                <div className="px-3 py-1 text-[11px] uppercase tracking-wide text-[var(--text-tertiary)]">
                  Inventory Fix + Sync
                </div>
                <button
                  type="button"
      ...
      {isColumnPanelOpen && (
        <div className="rounded-lg border border-[var(--border)] bg-[var(--bg)] p-4 space-y-2">
          <div className="flex items-center justify-between">
            <p className="text-sm font-semibold text-[var(--text-primary)]">{t('table.columns.visible')}</p>
            <button type="button" className="text-xs text-[var(--avy-purple-light)] hover:underline" onClick={resetColumns}>
              {t('table.columns.reset')}
            </button>
          </div>
          <div className="grid grid-cols-2 gap-2 max-h-48 overflow-y-auto">
            {columnDefinitions.map((column) => (
              <label key={column.id} className="flex items-center gap-2 text-sm text-[var(--text-secondary)]">
                <input
                  type="checkbox"
                  checked={visibleColumns.includes(column.id)}
                  onChange={() => toggleColumnVisibility(column.id)}
                  disabled={visibleColumns.length === 1 && visibleColumns.includes(column.id)}
                  className="bg-[var(--surface-hover)] border-[var(--border)]"
                />
                {column.label}
              </label>
            ))}
  ...
  const renderSelectionBar = () => {
    return (
      <div className="rounded-xl border border-[var(--border)] bg-[var(--bg)]/30 p-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <details className="relative">
            <summary className="cursor-pointer select-none list-none [&::-webkit-details-marker]:hidden rounded-md bg-[var(--surface)] border border-[var(--border)] px-3 py-2 text-xs font-semibold text-[var(--text-primary)] hover:border-[var(--border)]">
              Mehr
            </summary>
            <div className="absolute right-0 mt-2 w-[320px] max-w-[90vw] rounded-xl border border-[var(--border)] bg-[var(--bg)] p-1 shadow-[var(--shadow-lg)] z-30">
              <button
                type="button"
  ...
  return (
    <>
      <section id="admin-table" className="space-y-4">
        ...
        <div className="rounded-2xl border border-white/10 bg-[var(--bg)]/40 p-4 space-y-3">
          <div className="flex flex-wrap items-center gap-3">
            <div className="relative flex-1 min-w-[220px]">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2 bg-[var(--surface)] border border-[var(--border)] rounded-lg focus:ring-2 focus:ring-[var(--avy-purple)] focus:border-[var(--avy-purple)] text-sm"
              />
            </div>
            <div className="flex items-center gap-2 text-xs text-[var(--text-tertiary)]">
              ...
              <button type="button" onClick={resetFilters} className="text-[var(--avy-purple-light)] hover:underline">
                Filter zurücksetzen
              </button>
            </div>
          </div>
          {isMobile ? (
            <div className="rounded-2xl border border-[var(--border)] bg-[var(--bg)]/40">
              <button
                type="button"
                onClick={() => setMobileFiltersOpen((prev) => !prev)}
                className="w-full px-4 py-2 text-sm font-semibold text-[var(--text-primary)] flex items-center justify-between"
              >
            ...
            <div className="flex flex-wrap items-center gap-2">
              {activeFilterChips.map((chip) => (
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-full border border-[var(--border)] bg-[var(--bg)]/30 px-3 py-1 text-xs text-[var(--text-secondary)] hover:border-[var(--border)]"
        ...
        {filteredAndSortedProducts.length === 0 ? (
          <div className="rounded-xl bg-[var(--bg)]/40 p-4 text-sm text-[var(--text-secondary)] ring-1 ring-[var(--border)]/50">
            {mode === 'inventory' ? (
              <>
              ...
              <>
                ...
                <div className="mt-1 text-[var(--text-tertiary)]">Prüfe Suche/Filter oder ob alle Produkte bereits Inventory-Kriterien erfüllen.</div>
              </>
            ) : (
        ...
        <div className="overflow-x-auto">
          <table id="grid" className="w-full text-left min-w-[1000px]">
            <thead className="bg-[var(--surface-hover)]/50">
              <tr>
                <th className="p-3 w-12 text-xs font-semibold uppercase tracking-wide text-[var(--text-secondary)]">
                  <input
                    type="checkbox"
                ...
                <th className="p-3 text-xs font-semibold uppercase tracking-wide text-[var(--text-secondary)] whitespace-nowrap">
            ...
            <tbody>
              {pageProducts.map(p => (
                <tr
                  ...
                  className="border-b border-[var(--border)] hover:bg-[var(--surface-hover)]/50 transition-colors"
                >
                  <td className="p-3">
                    <input
                      type="checkbox"
        ...
        <div className="mt-4 flex flex-col sm:flex-row items-center justify-between gap-4 text-sm text-[var(--text-tertiary)]">
          <div className="flex items-center gap-2">
            <span>Zeige</span>
            <select
              ...
              className="bg-[var(--surface-hover)] border border-[var(--border)] rounded px-2 py-1 text-[var(--text-secondary)] focus:ring-2 focus:ring-[var(--avy-purple)] outline-none"
            >
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex gap-1">
              <button
                ...
                className="px-3 py-1 rounded bg-[var(--surface-hover)] hover:bg-[var(--surface-hover)] disabled:opacity-50 disabled:cursor-not-allowed transition-colors text-[var(--text-secondary)]"
              >
                Zurück
              </button>
              <button
      ...
      {inventoryModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4">
          <div className="w-full max-w-md rounded-2xl bg-[var(--bg)] border border-[var(--border)] p-5 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-semibold text-[var(--text-primary)]">{t('table.inventory.assignTitle')}</h3>
              <button
                type="button"
            ...
            <div className="space-y-2">
              <label className="block text-xs uppercase tracking-wide text-[var(--text-tertiary)]">
            ...
            {inventoryAssignMessage && (
              <p className="text-xs text-[var(--text-secondary)]">{inventoryAssignMessage}</p>
            )}
            <div className="flex justify-end gap-2">
              <button
                ...
                className="px-3 py-1.5 rounded-lg border border-[var(--border)] text-sm text-[var(--text-secondary)]"
              >
              ...
              <button
                ...
                className="px-4 py-1.5 rounded-lg bg-[var(--avy-purple)] text-sm font-semibold text-[var(--text-primary)] hover:bg-[var(--avy-purple-hover)] disabled:opacity-60"
              >
      ...
      {ktypeModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4">
          <div className="w-full max-w-2xl rounded-2xl bg-[var(--bg)] border border-[var(--border)] p-5 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-semibold text-[var(--text-primary)]">K‑Typ Import (CSV)</h3>
              <button
                type="button"
                className="text-[var(--text-tertiary)] hover:text-[var(--text-primary)]"
                onClick={() => {
            ...
            {ktypeMessage && (
              <p className="text-xs text-[var(--text-secondary)]">{ktypeMessage}</p>
            )}
            ...
            <div className="flex justify-end gap-2">
              <button
                ...
                className="px-3 py-1.5 rounded-lg border border-[var(--border)] text-sm text-[var(--text-secondary)] disabled:opacity-60"
              >
                Dry-Run
              </button>
              <button
            ...
            {ktypeReport && (
              <div className="rounded-xl border border-[var(--border)] bg-[var(--bg)]/40 p-3 space-y-2">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
                  <div className="text-[var(--text-secondary)]">
                    <div className="text-[var(--text-tertiary)]">SKUs</div>
                    <div className="font-semibold text-[var(--text-primary)]">{ktypeReport.parsed?.skus ?? '—'}</div>
                  </div>
                  <div className="text-[var(--text-secondary)]">
                    <div className="text-[var(--text-tertiary)]">Einträge</div>
                    <div className="font-semibold text-[var(--text-primary)]">{ktypeReport.parsed?.entries ?? '—'}</div>
                  </div>
                  <div className="text-[var(--text-secondary)]">
                    <div className="text-[var(--text-tertiary)]">Updated</div>
                    <div className="font-semibold text-[var(--text-primary)]">{ktypeReport.updated ?? 0}</div>
                  </div>
                  <div className="text-[var(--text-secondary)]">
                    <div className="text-[var(--text-tertiary)]">Not found</div>
                    <div className="font-semibold text-[var(--text-primary)]">{(ktypeReport.notFound || []).length}</div>
                  </div>
                </div>
                ...
                <details className="text-xs">
                  ...
                  <pre className="mt-2 max-h-72 overflow-auto whitespace-pre-wrap break-words text-[11px] text-[var(--text-secondary)]">
      ...
      {syncInProgress && (
        <div className="fixed bottom-6 right-6 z-40 flex items-center gap-3 rounded-2xl bg-[var(--bg)]/90 border border-[var(--border)] px-4 py-3 shadow-[var(--shadow-lg)] shadow-black/40 max-w-sm">
          <Spinner className="w-6 h-6 text-[var(--avy-purple-light)]" />
          <div className="text-sm text-[var(--text-primary)]">
            <p className="font-semibold">Sync läuft …</p>
            <p className="text-[var(--text-tertiary)] text-xs">{syncMessage || 'Produkte werden übertragen'}</p>
          </div>
        </div>
      )}
      {improveInProgress && (
        <div className="fixed bottom-20 right-6 z-40 flex items-center gap-3 rounded-2xl bg-[var(--bg)]/90 border border-[var(--border)] px-4 py-3 shadow-[var(--shadow-lg)] shadow-black/40 max-w-sm">
          <Spinner className="w-6 h-6 text-purple-300" />
          <div className="text-sm text-[var(--text-primary)]">
            <p className="font-semibold">Improve läuft …</p>
            <p className="text-[var(--text-tertiary)] text-xs">{improveMessage || 'Produkte werden verbessert'}</p>
          </div>
        </div>
      )}
    </>
