      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <div className="rounded-xl border border-[var(--border)] bg-[var(--surface)] p-5 space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold text-[var(--text-primary)]">Scopes</h3>
            <button
              type="button"
              onClick={loadScopes}
              disabled={loading}
          ...
          <div className="space-y-1">
            {scopes.map((s) => (
              ...
              >
                <div className="font-semibold text-[var(--text-primary)]">{s.name || s.id}</div>
                <div className="text-xs text-[var(--text-tertiary)]">{s.id}</div>
              </button>
            ))}
            {scopes.length === 0 && <div className="text-sm text-[var(--text-tertiary)]">Keine Scopes.</div>}
          </div>
        </div>

        <div className="lg:col-span-2 rounded-xl border border-[var(--border)] bg-[var(--surface)] p-5 space-y-4">
          <div className="space-y-1">
            <div className="text-sm font-semibold text-[var(--text-primary)]">{scope?.name || scope?.id || '\u2014'}</div>
            <div className="text-xs text-[var(--text-tertiary)]">{scope?.purpose || ''}</div>
            <div className="text-xs text-[var(--text-tertiary)]">
              Default Model Env: <code className="text-[var(--text-secondary)]">{scope?.defaultModelEnvKey || '\u2014'}</code> · Active Version:{' '}
              <code className="text-[var(--text-secondary)]">{scope?.activeVersionId || '\u2014'}</code>
            </div>
          </div>

          {scope?.activeVersionId && (
            <div className="rounded-lg border border-[var(--border)] bg-[var(--surface-secondary)] p-3 text-xs text-[var(--text-secondary)]">
              <div className="flex flex-wrap gap-3 items-center">
                <span className="font-semibold text-[var(--text-primary)]">Aktive Version:</span>
                <span className="text-[var(--text-primary)]">{scope.activeVersionId}</span>
              </div>
              <p className="mt-2 text-[var(--text-tertiary)]">
                Felder unten sind mit der aktuell aktiven Version vorbelegt. Aenderungen werden als neue Version gespeichert
          ...
          <div className="rounded-lg border border-[var(--border)] bg-[var(--bg)] p-4 space-y-3">
            <div className="flex flex-wrap gap-3">
              <label className="text-xs text-[var(--text-secondary)]">
                Prompt Mode
                <select
                  className="ml-2 bg-[var(--bg)] border border-[var(--border)] rounded-lg px-2 py-1 text-sm text-[var(--text-primary)] focus:border-[var(--avy-purple)] outline-none transition-colors duration-200"
                  value={promptMode}
            ...
            <label className="block text-xs text-[var(--text-secondary)] space-y-1">
              ...
              <textarea
                ...
                rows={6}
                className="w-full rounded-lg bg-[var(--bg)] border border-[var(--border)] px-3 py-2 text-sm text-[var(--text-primary)] outline-none focus:border-[var(--avy-purple)] font-mono text-xs transition-colors duration-200"
              />
            </label>
            <label className="block text-xs text-[var(--text-secondary)] space-y-1">
            ...
            <label className="block text-xs text-[var(--text-secondary)] space-y-1">
              ...
              <input
                ...
                className="w-full rounded-lg bg-[var(--bg)] border border-[var(--border)] px-3 py-2 text-sm text-[var(--text-primary)] outline-none focus:border-[var(--avy-purple)] transition-colors duration-200"
              />
            </label>
            <button
              type="button"
              ...
              className="rounded-lg bg-[var(--avy-purple)] hover:bg-[var(--avy-purple-hover)] disabled:opacity-50 disabled:cursor-not-allowed px-4 py-2 text-sm font-semibold text-white transition-all duration-200"
            >
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-2 max-h-[260px] overflow-auto">
              {versions.map((v: any) => (
                <div key={v.id} className="rounded-lg border border-[var(--border)] bg-[var(--bg)] p-3">
                  <div className="flex items-center justify-between gap-3">
                    <div className="text-xs text-[var(--text-secondary)]">
                      <div>
                        <code className="text-[var(--text-primary)]">{v.id}</code> {scope?.activeVersionId === v.id ? <span className="text-[var(--avy-purple)] font-semibold">ACTIVE</span> : null}
                      </div>
                      {v.note ? <div className="text-[var(--text-tertiary)]">{v.note}</div> : null}
                    </div>
                    <button
                      type="button"
                      onClick={() => activate(v.id)}
                      disabled={saving || scope?.activeVersionId === v.id}
                      className="rounded-lg border border-[var(--border)] bg-[var(--surface)] px-3 py-1.5 text-xs font-semibold text-[var(--text-primary)] hover:border-[var(--border-hover)] hover:bg-[var(--surface-secondary)] disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-150"
                    >
                      Activate
                    </button>
                  </div>
                  <div className="mt-2 grid grid-cols-1 md:grid-cols-2 gap-2 text-xs">
                    <div className="text-[var(--text-tertiary)]">
                      promptMode: <code className="text-[var(--text-secondary)]">{v.promptMode || 'append'}</code>
                    </div>
                    <div className="text-[var(--text-tertiary)]">
                      rulesMode: <code className="text-[var(--text-secondary)]">{v.rulesMode || 'append'}</code>
                    </div>
                  </div>
                </div>
              ))}
              {versions.length === 0 && <div className="text-sm text-[var(--text-tertiary)]">Noch keine Versionen.</div>}
            </div>
          </div>
        </div>
      </div>
