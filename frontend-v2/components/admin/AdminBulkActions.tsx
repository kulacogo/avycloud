    ...
    <div className="space-y-5">
      <div className="rounded-xl border border-[var(--border)] bg-[var(--surface)] p-5">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-[var(--text-secondary)]">
              Konsolidierte Bulk-Tasks fuer Produkt-Details (Titel/Preis/Kategorie). Laeuft asynchron als Admin-Job.
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="mt-4 grid grid-cols-1 md:grid-cols-6 gap-3">
          <label className="flex flex-col gap-1 text-sm text-[var(--text-secondary)] md:col-span-2">
            <span className="text-[11px] font-semibold uppercase tracking-wide text-[var(--text-tertiary)]">Action</span>
            <select
              value={action}
          ...
          <label className="flex items-center gap-2 text-sm text-[var(--text-secondary)] md:col-span-1">
          ...
          <label className="flex flex-col gap-1 text-sm text-[var(--text-secondary)]">
            <span className="text-[11px] font-semibold uppercase tracking-wide text-[var(--text-tertiary)]">offset</span>
            <input
              type="number"
              min={0}
              value={offset}
        ...
        {action === 'price' ? (
          <div className="mt-3 grid grid-cols-1 md:grid-cols-3 gap-3">
            <label className="flex flex-col gap-1 text-sm text-[var(--text-secondary)]">
              <span className="text-[11px] font-semibold uppercase tracking-wide text-[var(--text-tertiary)]">maxAgeDays (0 = nur missing)</span>
              <input
                type="number"
                min={0}
                max={365}
        ...
        <div className="rounded-xl border border-[var(--border)] bg-[var(--surface)] p-5 space-y-3">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-[var(--text-tertiary)] font-mono">{jobId}</p>
            </div>
            <button
              type="button"
              onClick={refreshJob}
              className="inline-flex items-center gap-2 rounded-lg border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-sm font-semibold text-[var(--text-secondary)] hover:border-[var(--border-hover)] hover:bg-[var(--surface-secondary)] transition-colors duration-150"
            >
              Refresh
            </button>
          </div>
          {job ? (
            <pre className="overflow-auto whitespace-pre-wrap rounded-lg border border-[var(--border)] bg-[var(--bg)] p-3 text-xs text-[var(--text-secondary)]">
