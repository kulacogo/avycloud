    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        ...
        <button
          ...
          onClick={load}
          disabled={loading}
          className="rounded-lg border border-[var(--border)] bg-[var(--surface)] px-4 py-2 text-sm font-semibold text-[var(--text-primary)] hover:border-[var(--border-hover)] hover:bg-[var(--surface-secondary)] disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-150"
        >
          Refresh
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-5">
          {roles.map((role) => {
            ...
            return (
              <div key={role.id} className="rounded-xl border border-[var(--border)] bg-[var(--surface)] p-5 space-y-4">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    <h3 className="text-sm font-semibold text-[var(--text-primary)]">{role.id}</h3>
                    <p className="text-xs text-[var(--text-tertiary)]">{role.name || ''}</p>
                  </div>
                  <button
                    type="button"
                ...
                <div className="overflow-auto">
                  <table className="min-w-full text-sm">
                    <thead>
                      <tr>
                        <th className="text-left py-3 px-4 text-xs font-semibold text-[var(--text-tertiary)] uppercase tracking-wide border-b border-[var(--border)]">Modul</th>
                        <th className="text-left py-3 px-4 text-xs font-semibold text-[var(--text-tertiary)] uppercase tracking-wide border-b border-[var(--border)]">Rechte</th>
                      </tr>
                    </thead>
                    <tbody>
                      {MODULES.map((mod) => (
                        <tr key={mod.id} className="border-b border-[var(--border)] last:border-b-0 hover:bg-[var(--surface-hover)] transition-colors duration-100">
                          <td className="py-3 px-4 text-[var(--text-secondary)] whitespace-nowrap">{mod.label}</td>
                          <td className="py-3 px-4">
                            <div className="flex flex-wrap gap-3">
                              {mod.actions.map((action) => (
                                <label key={action} className="flex items-center gap-2 text-xs text-[var(--text-primary)]">
                                  <input
                                    type="checkbox"
          ...
          {roles.length === 0 && <div className="text-sm text-[var(--text-tertiary)]">Keine Rollen gefunden.</div>}
        </div>
      )}
    </div>
  );
