...
const ChatContainer: React.FC<ChatContainerProps> = ({ children, onFilesDropped }) => {
  ...
  return (
    <aside
      ref={containerRef}
      className={`relative flex min-h-[420px] h-full flex-col overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--surface)] text-[var(--text-primary)] ${
    ...
    >
      {!isCompact && (
        <div
          ...
          className="absolute left-0 top-0 h-full w-1 cursor-col-resize rounded-l-xl bg-transparent transition hover:bg-[var(--avy-purple)]/40"
