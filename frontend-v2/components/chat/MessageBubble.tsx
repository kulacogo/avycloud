...
const MessageBubble: React.FC<MessageBubbleProps> = ({
  ...
  return (
    <div className={`flex ${role === 'user' ? 'justify-end' : 'justify-start'}`}>
      <div className={`max-w-[85%] rounded-xl px-4 py-3 text-sm shadow-sm ${bubbleClasses}`}>
        <div className="space-y-3">
          {segments.map((segment, index) => {
            if (segment.type === 'code') {
              return (
                <div key={`code-${index}`} className="relative rounded-lg bg-[var(--surface-secondary)] p-3 font-mono text-xs text-[var(--text-primary)]">
                  ...
                  <button
                    ...
                    className="absolute right-2 top-2 rounded-md bg-[var(--surface)] border border-[var(--border)] px-2 py-1 text-[10px] text-[var(--text-secondary)] hover:border-[var(--border-hover)] transition-colors duration-150"
                  >
                    Copy
                  </button>
                </div>
                ...
                {parts.map((part, idx) => {
                  ...
                  return isUrl ? (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="break-words text-[var(--avy-purple-light)] underline hover:text-[var(--avy-purple)] transition-colors duration-150"
                    >
                      {part}
                    </a>
                  ) : (
          ...
          {role === 'assistant' && datasheetChanges.length > 0 && (
            <div className="space-y-2 rounded-xl border border-[var(--border)] bg-[var(--surface-secondary)] p-3">
              <p className="text-[11px] font-semibold uppercase tracking-wide text-[var(--text-tertiary)]">Übernehmbare Daten</p>
              {datasheetChanges.map((entry) => {
                ...
                return (
                  <div key={entry.id} className="flex flex-col gap-2 rounded-lg bg-[var(--surface)] p-3 text-xs text-[var(--text-secondary)]">
                    <div>
                      <p className="text-sm font-semibold text-[var(--text-primary)]">{entry.change.summary || 'Änderung aus Chat'}</p>
                      {fieldKeys.length > 0 && (
                        <p className="text-[11px] text-[var(--text-tertiary)]">Felder: {fieldKeys.join(', ')}</p>
                      )}
                      {proposedTitle && (
                        <div className="mt-2 rounded-lg bg-[var(--surface-secondary)] p-2 ring-1 ring-[var(--border)]">
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <span
                              className={`text-[11px] font-semibold ${
                                titleLen > 80 ? 'text-[var(--error)]' : titleLen < 55 ? 'text-[var(--warning)]' : 'text-[var(--text-secondary)]'
                              }`}
                          ...
                          <p className="mt-1 text-sm text-[var(--text-primary)]">{proposedTitle}</p>
                        </div>
                      )}
                    </div>
                    {onApplyDatasheetChange && (
                      <button
                        ...
                        className="self-start rounded-full bg-[var(--avy-purple)] px-3 py-1 text-[11px] font-semibold text-white hover:bg-[var(--avy-purple-hover)] disabled:cursor-wait disabled:opacity-60 transition-colors duration-150"
                      >
          ...
          {attachments.length > 0 && (
            <details className="rounded-xl border border-[var(--border)] bg-[var(--surface-secondary)]">
              <summary className="cursor-pointer px-3 py-2 text-xs font-semibold uppercase tracking-wide text-[var(--text-tertiary)]">
        ...
        <span className="mt-2 block text-right text-[10px] uppercase tracking-wide text-[var(--text-tertiary)]">
