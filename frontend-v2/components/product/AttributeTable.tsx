...
const AttributeTable: React.FC<AttributeTableProps> = ({ attributes, isEditing = false, onChange, highlightKeys }) => {
  ...
  if (displayEntries.length === 0) {
    return (
      <div>
        <p className="text-[var(--text-tertiary)] text-sm">No specific attributes available.</p>
        {isEditing && (
          <button
            onClick={addRow}
            className="mt-3 px-3 py-1.5 text-xs font-semibold bg-[var(--surface-secondary)] text-[var(--text-primary)] rounded-lg border border-[var(--border)] hover:border-[var(--border-hover)] transition-colors"
          >
            + Add Attribute
          </button>
        )}
    ...
    <div className="overflow-x-auto">
        ...
        <tbody className="divide-y divide-[var(--border)]">
          {displayEntries.map(([key, value]) => (
            <tr
              key={key}
              className={
                highlightSet.has(String(key || '').toLowerCase())
                  ? 'bg-[var(--warning-bg)]'
                  : ''
              }
            >
              <td
                className={`py-3 pr-4 font-medium w-1/3 text-sm ${
                  highlightSet.has(String(key || '').toLowerCase())
                    ? 'text-[var(--warning)]'
              ...
              >
                {isEditing ? (
                  <input
                    ...
                    className={`w-full bg-[var(--bg)] border rounded-lg px-3 py-2 text-sm text-[var(--text-primary)] focus:outline-none focus:border-[var(--avy-purple)] focus:shadow-[var(--shadow-focus)] transition-all ${
                      highlightSet.has(String(key || '').toLowerCase())
                        ? 'border-[var(--warning)]'
                        : 'border-[var(--border)]'
                    }`}
              ...
              <td className="py-3 pl-4 text-sm text-[var(--text-primary)]">
                {isEditing ? (
                  <input
              ...
              {isEditing && (
                <td className="py-3 pl-4 text-right w-24">
                  <button
                    ...
                    className="px-2.5 py-1 text-xs font-semibold bg-[var(--error-bg)] text-[var(--error)] border border-[var(--error-border)] rounded-lg hover:bg-[var(--error)] hover:text-white transition-colors"
                  >
                    Remove
                  </button>
                </td>
      ...
      {isEditing && (
        <div className="mt-3">
          <button
            onClick={addRow}
            className="px-3 py-1.5 text-xs font-semibold bg-[var(--surface-secondary)] text-[var(--text-primary)] rounded-lg border border-[var(--border)] hover:border-[var(--border-hover)] transition-colors"
          >
            + Add Attribute
          </button>
        </div>
