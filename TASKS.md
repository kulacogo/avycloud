...
- [x] **Gap C** — ~~`backend/routes/marketplace.js:966` schreibt `inventory.quantity` direkt via `batch.update()` (Kaufland-Reconcile-Pfad). Verstößt gegen CLAUDE.md Punkt 10/13.~~ **RESOLVED (2026-06-28)** — der Reconcile-Direct-Write existiert nicht mehr; `marketplace.js:966` ist heute eine read-only `kauflandUnitsLive`-Query. Abgesichert durch `backend/__tests__/oversell-invariant.test.js` (greppt marketplace.js + kaufland-listings-sync.js gegen `reconBatch.update(...inventory.quantity)`).
